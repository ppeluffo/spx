...
UART_device_control_t *pxNewUart;
	...
	pxPeripheralControl->xBlockTime = (10 / portTICK_RATE_MS );
...
#ifdef DEBUG_I2C
		FRTOS_snprintf_P( stdout_buff,CHAR128,PSTR("FRTOS_I2C_IOCTL: 0x%02x,0x%02x\r\n\0"),(uint8_t)ulRequest, (uint8_t)(*p));
		CMD_write( stdout_buff, sizeof(stdout_buff) );
#endif
		switch( ulRequest )
		{
...
SPI_device_control_t *pxNewSPI;
	...
	pxPeripheralControl->xBlockTime = (50 / portTICK_RATE_MS );
	//
	pxNewSPI = ( int8_t * ) pvPortMalloc( sizeof(SPI_device_control_t ));
	pxPeripheralControl->phDevice = pxNewSPI;
...
//------------------------------------------------------------------------------------
size_t FreeRTOS_SPI_write( Peripheral_Descriptor_t const pxPeripheral, const void *TXBuffer, const size_t TXBytes )
{
size_t xReturn = 0U;

	// Envio el buffer.
	SPI_TransceivePacket( (char *)TXBuffer, TXBytes, NULL, 0);

	return(xReturn);

}
//------------------------------------------------------------------------------------
size_t FreeRTOS_SPI_read( Peripheral_Descriptor_t const pxPeripheral, void * const RXBuffer, const size_t RXbytes )
{

size_t xReturn = 0U;

	SPI_TransceivePacket( NULL,0 , (char *)RXBuffer, RXbytes);
	//
	return(xReturn);
...
uint16_t *p;
		...
		#ifdef DEBUG_SPI
				FRTOS_snprintf_P( stdout_buff,CHAR128,PSTR("FRTOS_SPI_IOCTL: 0x%02x,0x%02x\r\n\0"),(uint8_t)ulRequest, (uint8_t)(*p));
				CMD_write( stdout_buff, sizeof(stdout_buff) );
		#endif
		...
		switch( ulRequest )
		{
			...
			case ioctlSET_TIMEOUT:
				pxPeripheralControl->xBlockTime = *p;
				break;
			case ioctl_SPI_COMMAND:						// Comando a mandar al dispositivo SPI
				pSPI->spi_command = (int8_t)(*p);
				break;
			case ioctl_SPI_PAGEADDRESS:					// Seteo la direccion de la pagina en el dispositivo SPI
				pSPI->spi_page_address = (uint16_t)(*p);
				break;
			case ioctl_SPI_FLASH_ASSERT_CS:
				//MEMSPI_CS_PORT.OUT &= ~(_BV(MEMSPI_CS_PIN));	//	SPIFLASH_ASSERT_CS (Low)
				IO_clr_SPIMEM_CS();
				break;
			case ioctl_SPI_FLASH_RELEASE_CS:
				//MEMSPI_CS_PORT.OUT |= _BV(MEMSPI_CS_PIN);		//	SPIFLASH_RELEASE_CS ( High )
				IO_set_SPIMEM_CS();
				break;
			default :
				xReturn = pdFAIL;
