...
#endif
#if SYSMAINCLK == 8
		USARTF0.BAUDCTRLA = (uint8_t) 983;
		USARTF0.BAUDCTRLB = ( -7 << USART_BSCALE0_bp)|(983 >> 8);
		USARTF0.CTRLB |= USART_CLK2X_bm;
#endif
#if SYSMAINCLK == 2
		USARTF0.BAUDCTRLA = (uint8_t) 11;
		USARTF0.BAUDCTRLB = ( -7 << USART_BSCALE0_bp)|(11 >> 8);
#endif
...
#endif
#if SYSMAINCLK == 8
		USARTE0.BAUDCTRLA = (uint8_t) 983;
		USARTE0.BAUDCTRLB = ( -7 << USART_BSCALE0_bp)|(983 >> 8);
		USARTE0.CTRLB |= USART_CLK2X_bm;
#endif
#if SYSMAINCLK == 2
		USARTE0.BAUDCTRLA = (uint8_t) 11;
		USARTE0.BAUDCTRLB = ( -7 << USART_BSCALE0_bp)|(11 >> 8);
#endif
...
//		USARTC0.BAUDCTRLB = ( -7 << USART_BSCALE0_bp)|(2094 >> 8);
		...
		USARTC0.BAUDCTRLB = ( -4 << USART_BSCALE0_bp)|(3317 >> 8);
#endif
#if SYSMAINCLK == 8
//		USARTC0.BAUDCTRLA = (uint8_t) 983;
//		USARTC0.BAUDCTRLB = ( -7 << USART_BSCALE0_bp)|(983 >> 8);
//		USARTC0.CTRLB |= USART_CLK2X_bm;
		USARTC0.BAUDCTRLA = (uint8_t) 3269;
		USARTC0.BAUDCTRLB = ( -6 << USART_BSCALE0_bp)|(3269 >> 8);
#endif
#if SYSMAINCLK == 2
//		USARTC0.BAUDCTRLA = (uint8_t) 11;
//		USARTC0.BAUDCTRLB = ( -7 << USART_BSCALE0_bp)|(11 >> 8);
		USARTC0.BAUDCTRLA = (uint8_t) 3205;
		USARTC0.BAUDCTRLB = ( -7 << USART_BSCALE0_bp)|(3205 >> 8);
#endif
...
/*------------------------------------------------------------------------------------*/
uint16_t print_gprs_fifo_meswaitint(void)
{
...
uint16_t msg_waiting;
	...
	pUart = pdUART_GPRS.phDevice;
	pxFifo = pUart->rxStruct;
	//msg_waiting = pxFifo->uxMessageWaiting;
	msg_waiting = uxFifoMessagesWaiting(pxFifo);
...
//------------------------------------------------------------------
/*
void uxSwitchBuffers(fifoHandle_t xFifo)
{

fifo_handle_s *pxFifo = xFifo;

	taskENTER_CRITICAL();
	//pxFifo->uartKernel = (!pxFifo->uartKernel) & 0x01;
	//pxFifo->uartUser = (!pxFifo->uartUser) & 0x01;

	if ( pxFifo->uartKernel == 0 ) {
		pxFifo->uartKernel = 1;
		pxFifo->uartUser = 0;
	} else {
		pxFifo->uartKernel = 0;
