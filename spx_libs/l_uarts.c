...
size_t uXBEE_read(  void *pvBuffer, const size_t xBytes )
{
	...
	portTickType xTicksToWait;
	xTimeOutType xTimeOut;
	UART_device_control_t *pUartXBEE;

	pUartXBEE = pdUART_XBEE.phDevice;

	xTicksToWait = pdUART_XBEE.xBlockTime;
	xTicksToWait = 5;
	vTaskSetTimeOutState( &xTimeOut );
	...
	while( xBytesReceived < xBytes )
	{
		/* Receive the next character. */
		// Los FIFO no tienen timeout, retornan enseguida
		if(  ( xFifoReceive( pUartXBEE->rxStruct, &((char *)pvBuffer)[ xBytesReceived ], xTicksToWait ) == pdPASS )) {
			xBytesReceived++;
			} else {
			// Espero xTicksToWait antes de volver a chequear
			vTaskDelay( ( TickType_t)( xTicksToWait ) );
