...
uint8_t xBytes = 0;
	...
	FreeRTOS_ioctl(&pdI2C,ioctl_I2C_SET_DEVADDRESS, & bus_address);
	...
	FreeRTOS_ioctl(&pdI2C,ioctl_I2C_SET_BYTEADDRESSLENGTH, &dev_address_length);
	// 	Direccion: El FRTOS espera siempre 2 bytes.
	//  Hago un cast para dejarla en 16 bits.
	device_address = (uint16_t)(rdAddress);
	FreeRTOS_ioctl(&pdI2C,ioctl_I2C_SET_BYTEADDRESS,&device_address);
